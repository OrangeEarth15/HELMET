#!/usr/bin/env python3
"""
收集GLM4 XAT attention方案的HELMET评估结果
专门用于汇总GLM-4-9B-Chat的full, xattn, flex, xflex等attention机制的结果
"""
...
def main():
    """收集GLM4 XAT attention结果"""
    
    # 获取HELMET根目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    helmet_root = os.path.dirname(script_dir)
    
    # 🎯 GLM4 XAT Attention配置 - 根据实际存在的目录调整
    glm4_configs = [
        # Full FlashAttention
        ...
        {"model": "GLM-4-9B-Chat", "tag": "glm4_flex_gamma0.95_tau0.1", 
         "output_dir": os.path.join(helmet_root, "glm4_output", "flex_gamma0.95_tau0.1"), "attention": "flex", "gamma": 0.95, "tau": 0.1},
    ]
    ...
    print(f"🎯 将处理 {len(glm4_configs)} 个GLM4 XAT attention配置")

    # 收集结果
    failed_paths = []
    df = []
    
    for config in tqdm(glm4_configs, desc="收集GLM4 XAT结果"):
        args = arguments()
        args.tag = config["tag"]
    ...
    for model in models:
        ...
        model_name = model.replace("GLM-", "glm").replace("-", "_").replace(".", "").lower()
