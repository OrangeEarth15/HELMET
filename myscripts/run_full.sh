...
# HELMET FlashInfer Full Attention 评估脚本 - 自定义缓存路径版本
echo "Running HELMET with FlashInfer Full Attention"
...
# for task in "rag" "longqa" "summ" "icl" "rerank" "cite"; do
...
echo "Running 128k versions with full attention"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with full attention"
    mkdir -p $OUTPUT_DIR/$task
    python eval.py \
...
echo "Full attention evaluation completed! Results in $OUTPUT_DIR"
