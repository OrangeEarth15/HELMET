...
# 设置输出目录
export OUTPUT_DIR="moe_qwen3_output/xflex_threshold0.95_scoreratio0.95"
mkdir -p $OUTPUT_DIR
...
echo "🎯 Attention: XFlex (threshold=0.95, score_ratio=0.95, stride=16)"
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    ...
    python eval.py \
        ...
        --attn_stride 16 \
        --tag qwen3_moe_xflex_threshold0.95_scoreratio0.95 \
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Qwen3 MoE xflex"
    python eval.py \
