...
# HELMET Qwen3-30B-A3B-Instruct MoE XAttention (threshold=0.95) 评估脚本
echo "Running HELMET with Qwen3-30B-A3B-Instruct MoE XAttention (threshold=0.95)"
...
# 设置输出目录
export OUTPUT_DIR="moe_qwen3_output/xattn_threshold0.95"
mkdir -p $OUTPUT_DIR
...
echo "🎯 Attention: XAttention (threshold=0.95, stride=16)"
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    ...
    python eval.py \
        ...
        --attn_threshold 0.95 \
        --attn_stride 16 \
        --tag qwen3_moe_xattn_threshold0.95 \
