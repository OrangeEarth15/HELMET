...
# HELMET Yi-9B-200K XAttention v6 评估脚本 - 128k版本 (threshold 0.95)
echo "Running HELMET with Yi-9B-200K XAttention v6 (128k versions, threshold=0.95)"
...
# 设置输出目录
export OUTPUT_DIR="yi_output/xattn_v6_threshold${THRESHOLD}_128k"
mkdir -p $OUTPUT_DIR

echo "Running 128k versions with Yi-9B-200K XAttention v6 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Yi-9B-200K XAttention v6 (threshold=$THRESHOLD, 128k version)"
    ...
    python eval.py \
        ...
        --tag yi_xattn_v6_threshold${THRESHOLD}_128k \
        --output_dir $OUTPUT_DIR/${task}_128k
done

echo "Yi-9B-200K XAttention v6 128k versions (threshold=$THRESHOLD) evaluation completed! Results in $OUTPUT_DIR"
