...
# HELMET Qwen2.5-MoE-A2.7B-Instruct XAttention 评估脚本 - threshold 0.95
echo "Running HELMET with Qwen2.5-MoE-A2.7B-Instruct XAttention (threshold=0.95)"
...
# 设置输出目录
export OUTPUT_DIR="moe_qwen3_output/xattn_threshold${THRESHOLD}"
mkdir -p $OUTPUT_DIR

echo "Running 128k versions with Qwen2.5-MoE-A2.7B-Instruct XAttention (threshold=$THRESHOLD, stride=$STRIDE)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Qwen2.5 MoE XAttention (threshold=$THRESHOLD)"
    ...
    python eval.py \
        ...
        --attn_stride $STRIDE \
        --tag qwen3_moe_xattn_threshold${THRESHOLD} \
        --output_dir $OUTPUT_DIR/$task
done

echo "Qwen2.5-MoE XAttention (threshold=$THRESHOLD) evaluation completed! Results in $OUTPUT_DIR"
