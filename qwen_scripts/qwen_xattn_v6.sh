...
THRESHOLD=0.95
STRIDE=8
USE_SIMPLE=6  # v6版本：golden ratio selection
...
echo "Running 8k to 64k versions with Qwen2.5-7B-Instruct XAttention v6 (threshold=0.95)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task (short) with Qwen2.5 XAttention v6 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
...
echo "Running 128k versions with Qwen2.5-7B-Instruct XAttention v6 (threshold=0.95)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Qwen2.5 XAttention v6 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
    mkdir -p $OUTPUT_DIR/$task
    python eval.py \
