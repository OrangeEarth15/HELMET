...
# HELMET Qwen2.5-7B-Instruct XFlex v6 评估脚本 - 8k到64k版本 (threshold 0.95, score_ratio 0.001)
echo "Running HELMET with Qwen2.5-7B-Instruct XFlex v6 (8k-64k versions, threshold=0.95, score_ratio=0.001)"
...
# 设置输出目录
export OUTPUT_DIR="qwen_output/xflex_v6_threshold${THRESHOLD}_score${SCORE_RATIO}_short"
mkdir -p $OUTPUT_DIR
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    ...
    python eval.py \
        ...
        --tag qwen_xflex_v6_threshold${THRESHOLD}_score${SCORE_RATIO}_short \
        --output_dir $OUTPUT_DIR/${task}_short
done

echo "Qwen2.5 XFlex v6 short versions (threshold=$THRESHOLD, score_ratio=$SCORE_RATIO) evaluation completed! Results in $OUTPUT_DIR"
