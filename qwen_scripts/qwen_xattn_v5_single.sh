...
# HELMET Qwen2.5-7B-Instruct XAttention v5 评估脚本 - threshold 0.95
echo "Running HELMET with Qwen2.5-7B-Instruct XAttention v5 (threshold=0.95)"
echo "💡 v5 = robin head selection"
...
THRESHOLD=0.95
STRIDE=8
USE_SIMPLE=5  # v5版本：robin head selection

# 设置输出目录
export OUTPUT_DIR="qwen_output/xattn_v5_single_threshold${THRESHOLD}"
mkdir -p $OUTPUT_DIR

echo "Running 8k to 64k versions with Qwen2.5-7B-Instruct XAttention v5 (threshold=$THRESHOLD)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task (short) with Qwen2.5 XAttention v5 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
    ...
    python eval.py \
        ...
        --tag qwen_xattn_v5_threshold${THRESHOLD} \
        --output_dir $OUTPUT_DIR/${task}
done

echo "Running 128k versions with Qwen2.5-7B-Instruct XAttention v5 (threshold=$THRESHOLD)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Qwen2.5 XAttention v5 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
...
echo "Qwen2.5 XAttention v5 (threshold=$THRESHOLD) evaluation completed! Results in $OUTPUT_DIR"
