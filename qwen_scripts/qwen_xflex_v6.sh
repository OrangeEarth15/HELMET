...
# HELMET Qwen2.5-7B-Instruct XFlex v6 评估脚本 - threshold 0.95, score_ratio 0.001
echo "Running HELMET with Qwen2.5-7B-Instruct XFlex v6 (threshold=0.95, score_ratio=0.001)"
...
# 设置输出目录
export OUTPUT_DIR="qwen_output/xflex_v6_threshold${THRESHOLD}_score${SCORE_RATIO}"
mkdir -p $OUTPUT_DIR
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with Qwen2.5 XFlex v6 (threshold=$THRESHOLD, score_ratio=$SCORE_RATIO)"
    mkdir -p $OUTPUT_DIR/$task
    python eval.py \
...
echo "Qwen2.5 XFlex v6 (threshold=$THRESHOLD, score_ratio=$SCORE_RATIO) evaluation completed! Results in $OUTPUT_DIR"
