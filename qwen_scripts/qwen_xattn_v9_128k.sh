...
# HELMET Qwen2.5-7B-Instruct XAttention v9 评估脚本 - threshold 0.95
echo "Running HELMET with Qwen2.5-7B-Instruct XAttention v9 (threshold=0.95)"
...
# 设置输出目录
export OUTPUT_DIR="qwen_output/xattn_v9_threshold${THRESHOLD}"
mkdir -p $OUTPUT_DIR

# echo "Running 8k to 64k versions with Qwen2.5-7B-Instruct XAttention v9 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
# for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
#     echo "Running task: $task (short) with Qwen2.5 XAttention v9 (threshold=$THRESHOLD)"
...
#         --tag qwen_xattn_v9_threshold${THRESHOLD} \
#         --output_dir $OUTPUT_DIR/$task
# done

echo "Running 128k versions with Qwen2.5-7B-Instruct XAttention v9 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
# for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
for task in "cite" "rerank" "icl" "summ" "longqa" "rag" "recall"; do
    echo "Running task: $task with Qwen2.5 XAttention v9 (threshold=$THRESHOLD)"
    mkdir -p $OUTPUT_DIR/$task
    python eval.py \
...
echo "Qwen2.5 XAttention v9 (threshold=$THRESHOLD) evaluation completed! Results in $OUTPUT_DIR"
