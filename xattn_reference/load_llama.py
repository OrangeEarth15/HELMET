...
# from xattn.threshold.llama_threshold_xattn import llama_fuse_8
import flashinfer
import time
try:
...
try:
    ...
    print("Minference Prefill Import Fail:", str(e))
    try:
        # Try importing from MInference project
        import sys
        sys.path.append('/home/scratch.sarawang_ent/project/MInference')
        from xattn.src.Minference import Minference_prefill
        print("Minference imported successfully from MInference path")
    except Exception as e2:
        print("Failed to import from MInference path:", str(e2))
...
# from xattn.src.Tattention import Tattention_prefill  # 添加TAttention导入
...
def forward_eval(
        ...
        **kwargs,
        ...
        if not decoding:
            ...
            # elif self.fastprefillconfig.metric == "tattn":  # 添加TAttention支持
            #     # 🎯 直接使用脚本级别传入的配置，无需内部动态计算
            #     if isinstance(self.fastprefillconfig.threshold, torch.Tensor):
            ...
            #             # global_mode=getattr(self.fastprefillconfig, 'global_mode', False)  # Commented out: using non-global mode only
            #         )
            #     else:
            #         attn_output = Tattention_prefill(
            #             query_states, key_states, value_states, stride, 
            ...
            #             # global_mode=getattr(self.fastprefillconfig, 'global_mode', False)  # Commented out: using non-global mode only
            #         )
            elif self.fastprefillconfig.metric == "xflex":  # 添加XFlex支持
                # 🎯 XFlex稀疏注意力 - 整合xattn和flexprefill
                if isinstance(self.fastprefillconfig.threshold, torch.Tensor):
                ...
                if 'Minference_prefill' in globals() and Minference_prefill is not None:
                    attn_output = Minference_prefill(query_states, key_states, value_states, adaptive_budget=0.3)
                else:
                    raise RuntimeError("Minference_prefill function is not available. Please check MInference installation and dependencies.")
        else:
...
class FastPrefillConfig(dict):
    ...
    def __init__(
        ...
        # global_mode: bool = False,  # Commented out: using non-global mode only
    ):
        """
        ...
        # self.global_mode = global_mode  # Commented out: using non-global mode only
...
def load_model(fastprefillconfig=None,name_or_path=""):
    """
        ...
        Returns:
        - model: The loaded LLaMA model with FastPrefill optimizations.
