...
# 设置输出目录
export OUTPUT_DIR="llama_output/xflex_v6_threshold${THRESHOLD}_score${SCORE_RATIO}"
mkdir -p $OUTPUT_DIR
...
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task (short) with LLaMA3.1 XFlex v6 (threshold=$THRESHOLD, score_ratio=$SCORE_RATIO)"
