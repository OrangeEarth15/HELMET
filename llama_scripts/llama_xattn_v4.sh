...
# HELMET LLaMA3.1-8B-Instruct XAttention v4 评估脚本 - threshold 0.95
echo "Running HELMET with LLaMA3.1-8B-Instruct XAttention v4 (threshold=0.95)"
echo "💡 v4 = golden ratio selection"
...
THRESHOLD=0.95
STRIDE=8
USE_SIMPLE=4  # v4版本：golden ratio selection

# 设置输出目录
export OUTPUT_DIR="llama_output/xattn_v4_threshold${THRESHOLD}"
mkdir -p $OUTPUT_DIR

echo "Running 8k to 64k versions with LLaMA3.1-8B-Instruct XAttention v4 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task (short) with LLaMA3.1 XAttention v4 (threshold=$THRESHOLD)"
...
echo "Running 128k versions with LLaMA3.1-8B-Instruct XAttention v4 (threshold=$THRESHOLD, stride=$STRIDE, use_simple=$USE_SIMPLE)"
for task in "recall" "rag" "longqa" "summ" "icl" "rerank" "cite"; do
    echo "Running task: $task with LLaMA3.1 XAttention v4 (threshold=$THRESHOLD)"
    mkdir -p $OUTPUT_DIR/$task
    python eval.py \
...
echo "LLaMA3.1 XAttention v4 (threshold=$THRESHOLD) evaluation completed! Results in $OUTPUT_DIR"
